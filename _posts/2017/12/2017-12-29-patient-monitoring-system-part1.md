...
features has been supported. One of the advantage of using BLE mesh in Zephyr
...
see what's going on and how the stack is working. And to our own benefits
they have included some sample applications and tests for making the lives
...
this mesh network special among other networking topologies. Blueooth uses this
topology to connect nodes within a network so that it can extend its coverage and
efficiently communicate with each nodes in a network.

BLE mesh network consists of nodes (BLE compatible devices) which takes part in the
network and each nodes consists of Elements, uniquely addressable entities in a
network. Each node has to be provisioned by a provisioner (Mobile/Laptop in most case)
inorder to take part in the mesh network. Provisioning involves assigning the
...
the behaviours of each nodes. There are Standard and Vendor specific models.
...
the client, it will send the 'Sensor Status' message in response. Then
client can use this response to find out the sensor readings.
...
co-processor. But it is recommended to use STM32 microcontroller
...
After board reset you should see the below messgage on the serial port:
...
Final step is to provision and configure both the nodes. This requires
...
Now, generate AppKey and bind it to the 0th element of Client Server model (1102).
```shell
[meshctl]# appkey-add 1
...
Below is the behaviour of the mesh network:
...
4. Client node receives the message from the server, it parses and prints the value in terminal.
...
a full fledged mesh implementation but rather a getting started demo.
