...
After the container is laucnhed we need to check its name with `docker ps` and `docker exec` on it followed by sourcing the `Autoware/install/setup.bash` file as we did with the Hikey970 for terminals 2 to 4.
