...
Up till now we have been using a separate laptop for visualization purposes when we needed. Since we want to fully use [arm](https://www.arm.com/) SoCs, next time, we will use both boards to fully run the rosbag demo without the need of a laptop, so keep an eye to this space.
