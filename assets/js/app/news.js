// News source
sources = ["https://www.linaro.org/assets/json/news.json"];
allJSONData = [];
...
function addLatestNews(selector, results_data, number_of_items) {
  ...
  for (var i = 0; i < number_of_items; i++) {
    post = results_data[i];
    listItems +=
      '<a href="' + post.url + '" class="list-group-item text-dark">';
    listItems += '<h4 class="official-news-title">' + post.title + "</h4>";
    listItems += '<span class="date">' + post.date + "</span>";
    listItems += "</a>";
  }
    ...
    $.ajax({
      ...
      complete: function (jsonResponse) {
        ...
        $.each(jsonData, function (idx, obj) {
          ...
          var in_keywords = checkArrayForKeyword(post_tags, "96boards");
          if (in_keywords === true) {
