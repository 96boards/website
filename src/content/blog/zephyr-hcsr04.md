...
- **Our next do while loop runs till the ECHO pin remains HIGH:** Inside this do while block, we calculate the number of cycles spent and constantly check a value using `if` to limit the max range. We use the formula (((MAX*RANGE * 58000) / 1000000000) \_ (CLOCK \* 1000000)) and replace MAX_RANGE with the needed value in cm and replace CLOCK with the clock speed of the Microprocessor.
